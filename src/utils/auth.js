...
const TOKEN_KEY = 'Egg_token'; //access token
const REFRESH_TOKEN_KEY = 'Egg_refresh_token';
const TOKEN_EXPIRES_KEY = 'Egg_token_expires_at';
const TOKEN_EXPIRY_BUFFER = 5 * 1000; //在真的過期前有5秒的刷新緩衝時間
const USER_INFO_KEY = 'Egg_user_info';
const USER_PERMISSION = 'Egg_permission';
...
export const removeUserInfo = () => {
  ...
  localStorage.removeItem('Egg_user_permissions'); //使用者權限（PermissionStore）
...
export const hasPermission = (permissionKey) => {
  ...
  //在所有系統中尋找權限
  return Object.values(userInfo.permission).some((system) =>
    Object.values(system).some((group) => group.page?.some((page) => page.key === permissionKey) || group.role?.some((role) => role.key === permissionKey)),
  );
};
