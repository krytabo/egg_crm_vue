      ...
      <AForm ref="formRef" :model="basicForm" :rules="basicFormRules" layout="vertical" auto-label-width>
        ...
        <div class="mb-4 grid gap-4 md:grid-cols-2">
          ...
          <AFormItem :label="t('countedBy', '盤點人員')" field="countedBy">
            <InfiniteSelect v-model="basicForm.countedBy" dataSource="employees" :placeholder="t('pleaseSelect', '請選擇')" />
          </AFormItem>
        </div>
          ...
          <div class="rounded-md border">
            <table class="w-full">
              <thead class="bg-gray-50">
                <tr>
                  <th class="px-4 py-2 text-left text-sm font-medium text-gray-600">{{ t("product", "商品") }}</th>
                  <th class="w-32 px-4 py-2 text-right text-sm font-medium text-gray-600">{{ t("systemQuantity", "系統數量") }}</th>
                  <th class="w-32 px-4 py-2 text-right text-sm font-medium text-gray-600">{{ t("countedQuantity", "盤點數量") }}</th>
                  <th class="w-32 px-4 py-2 text-right text-sm font-medium text-gray-600">{{ t("difference", "差異") }}</th>
                  <th class="w-16 px-4 py-2 text-center text-sm font-medium text-gray-600">{{ t("actions", "操作") }}</th>
                </tr>
              </thead>
              <tbody class="divide-y">
                <tr v-for="(item, index) in basicForm.items" :key="index">
                  <td class="px-4 py-2">
                    <InfiniteSelect v-model="item.productId" dataSource="products" :placeholder="t('pleaseSelect', '請選擇')" size="small" @change="(val) => handleProductChange(index, val)" />
                  </td>
                  <td class="px-4 py-2 text-right text-gray-500">{{ item.systemQuantity ?? "—" }}</td>
                  <td class="px-4 py-2">
                    <a-input-number v-model="item.countedQuantity" :min="0" size="small" class="w-full" @change="calculateDifference(index)" />
                  </td>
                  ...
                  <td class="px-4 py-2 text-center">
                    <a-button type="text" status="danger" size="small" @click="removeCountItem(index)">
                      <template #icon><Trash2 class="size-4" /></template>
                    </a-button>
                  </td>
                </tr>
                <tr v-if="basicForm.items.length === 0">
...
const handleProductChange = async (index, product) => {
  ...
  try {
    ...
    const response = await InventoryListGet({ productId, limit: 1 });
