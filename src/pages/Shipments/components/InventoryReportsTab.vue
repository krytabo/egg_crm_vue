...
<template>
  ...
  <!--          頁籤            -->
  <!--＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝-->
  <a-radio-group v-model="reportType" type="button" @change="handleReportTypeChange" class="w-fit">
  ...
  <template v-if="reportType === 'valuation'">
    ...
    <CustomForm :col="1">
      <CustomFormItem :label="t('location', '存放位置')">
        <InfiniteSelect
          v-model="valuationFilters.location"
          dataSource="InventoryLocations"
          type="outline"
          :placeholder="t('allLocations', '所有位置')"
          allowClear
          @change="loadValuationReport"
          class="w-60"
        />
      </CustomFormItem>
    </CustomForm>
    ...
    <!--         統計區塊         -->
    <!--＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝-->
    <div class="grid grid-cols-3 gap-3 rounded-md bg-[#f5f7fb] p-4">
    ...
    <!--          列表            -->
    <!--＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝-->
    <CustomTinyGrid :data="valuationList" :height="TableScrollY" :border="true" row-key="productId">
  ...
  <template v-if="reportType === 'turnover'">
    ...
    <!--          列表            -->
    <!--＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝-->
    <CustomTinyGrid :data="turnoverList" :height="TableScrollY" :border="true" row-key="productId">
      <CustomTinyGridColumn field="productName" :title="t('productName', '產品名稱')" min-width="240" fixed="left" />
      <CustomTinyGridColumn field="productCode" :title="t('productCode', '產品編號')" width="200" />
...
const TableScrollY = computed(() => Math.max(windowHeight.value - 470, 100));
