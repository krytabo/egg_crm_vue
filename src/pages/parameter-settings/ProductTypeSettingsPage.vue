  ...
  <Card ref="containerRef">
    ...
    <CardHeader>
      ...
      <div class="flex items-center gap-3">
        ...
        <Button type="primary" @click="openCreateDialog">{{ t("addProductType") }}</Button>
      </div>
    </CardHeader>
        ...
        <CustomTinyGridColumn :title="t('actions')" :width="200" fixed="right" align="center">
          <template #default="{ row }">
            <div class="flex items-center justify-center gap-2">
              <button class="table-button" :disabled="!row.isDeletable" @click="deleteData(row.id)"><Trash2 class="size-4 text-rose-500" /></button>
...
/** 列表資料取得相關 **/
const { basicDataList, filters, pagination, pageSizeOptions, getDefaultAPI, handleGlobalSearch, handleFiltersChange, clearFilter, CurrentChange, SizeChange } = usePaginatedSearchApi(wrappedProductTypeListGet, defaultFilters, {
  responseDataToList
