  ...
  <Card ref="containerRef">
    ...
    <CardHeader>
      ...
      <div class="flex items-center gap-3">
        ...
        <Button type="primary" @click="openCreateDialog">{{ t("addPermission") }}</Button>
      </div>
    </CardHeader>
      ...
      <AForm ref="basicFormRef" auto-label-width :model="basicForm" layout="vertical" :rules="basicFormRules">
        <div class="grid gap-4 md:grid-cols-2">
          <AFormItem :label="t('resourceWithHint')" field="resource">
            <CustomField v-model="basicForm.resource" type="select" :options="resourceOptions" :placeholder="t('pleaseSelectResource')" allowClear />
          </AFormItem>
...
const resourceOptions = [
  { label: "使用者管理", value: "USER" },
  { label: "角色管理", value: "ROLE" },
  { label: "客戶管理", value: "CUSTOMER" },
  { label: "訂單管理", value: "ORDER" },
  { label: "產品管理", value: "PRODUCT" },
  { label: "庫存管理", value: "INVENTORY" },
  { label: "帳務管理", value: "BILLING" },
  { label: "供應商管理", value: "VENDOR" },
  { label: "車輛管理", value: "VEHICLE" },
  { label: "司機管理", value: "DRIVER" },
  { label: "報表管理", value: "REPORT" },
  { label: "檔案管理", value: "FILE" },
  { label: "通知管理", value: "NOTIFICATION" },
  { label: "KPI 儀表板", value: "KPI" }
];
const actionOptions = [
  { label: "新增資料", value: "CREATE" },
  { label: "讀取 / 查詢資料", value: "READ" },
  { label: "更新 / 修改資料", value: "UPDATE" },
  { label: "刪除資料", value: "DELETE" },
  { label: "匯出資料", value: "EXPORT" },
  { label: "匯入資料", value: "IMPORT" }
];
const resourceFilterOptions = [{ label: t("all"), value: "" }, ...resourceOptions];
const actionFilterOptions = [{ label: t("all"), value: "" }, ...actionOptions];
const resourceLabelMap = Object.fromEntries(resourceOptions.map((item) => [item.value, item.label]));
const actionLabelMap = Object.fromEntries(actionOptions.map((item) => [item.value, item.label]));
const ownOnlyOptions = [
  { label: t("ownOnlyOptionAny"), value: "ANY" },
  { label: t("ownOnlyOptionOwn"), value: "OWN" }
];
...
const responseDataToList = (item = {}) => {
  ...
  return {
    ...
    resourceLabel: resourceLabelMap[item.resource] || item.resource || t("unset"),
    actionLabel: actionLabelMap[item.action] || item.action || t("unset"),
...
/** 列表資料取得相關 **/
const { basicDataList, filters, pagination, pageSizeOptions, getDefaultAPI, handleGlobalSearch, handleFiltersChange, clearFilter, CurrentChange, SizeChange } = usePaginatedSearchApi(wrappedPermissionListGet, defaultFilters, {
  responseDataToList
...
const dialogTitle = computed(() => (isCreate.value ? "新增權限" : "編輯權限")); //彈窗標題
