<!-- src/pages/order-management/BottledWaterOrdersPage.vue 桶裝水訂單管理 -->
<script setup>
...
const mockWaterProducts = [
  { id: 1, name: '清泉 20L', retailPrice: 150, unit: '桶', currentStock: 300 },
  { id: 2, name: '純淨 20L', retailPrice: 145, unit: '桶', currentStock: 250 },
  { id: 3, name: '山泉 12L', retailPrice: 110, unit: '桶', currentStock: 180 },
  { id: 4, name: '礦泉 20L', retailPrice: 160, unit: '桶', currentStock: 200 },
  { id: 5, name: '純水 18L', retailPrice: 135, unit: '桶', currentStock: 150 }
]
const initialOrders = [
  {
    id: 1,
    orderNumber: 'WATER-2025-001',
    ...
    targetCompany: '林氏企業',
    products: [
      { productId: 1, productName: '清泉 20L', quantity: 20, retailPrice: 150, actualPrice: 140, subtotal: 2800, unit: '桶' },
      { productId: 2, productName: '純淨 20L', quantity: 10, retailPrice: 145, actualPrice: 140, subtotal: 1400, unit: '桶' }
    ],
    totalAmount: 4200,
  ...
  },
  {
    ...
    targetName: '李美華',
    targetCompany: '黃記商行',
    products: [{ productId: 3, productName: '山泉 12L', quantity: 15, retailPrice: 110, actualPrice: 105, subtotal: 1575, unit: '桶' }],
    totalAmount: 1575,
  ...
  },
  {
    ...
    targetCompany: null,
    products: [
      { productId: 4, productName: '礦泉 20L', quantity: 30, retailPrice: 160, actualPrice: 160, subtotal: 4800, unit: '桶' },
      { productId: 5, productName: '純水 18L', quantity: 20, retailPrice: 135, actualPrice: 135, subtotal: 2700, unit: '桶' }
    ],
    totalAmount: 7500,
...
const getProductInfo = (productId) => mockWaterProducts.find((item) => item.id === productId) || null
...
const updateProductRow = (uid, field, value) => {
  formProducts.value = formProducts.value.map((product) => {
    ...
    if (field === 'productId') {
      ...
      const matched = mockWaterProducts.find((item) => item.id === next.productId)
...
const saveOrder = () => {
  ...
  const productsPayload = formProducts.value.map((product) => {
    const matched = mockWaterProducts.find((item) => item.id === product.productId)
    return {
      productId: product.productId,
  ...
  if (editOrderId.value) {
    ...
    orders.value = [
      {
        id: nextId,
        orderNumber: `WATER-${new Date().getFullYear()}-${sequence}`,
        orderDate: orderForm.orderDate,
        shipDate: orderForm.shipDate,
        targetType: orderForm.targetType,
        targetId: orderForm.targetId,
...
<template>
  <div class="space-y-6 p-6">
    <div>
      <h1 class="text-2xl font-semibold text-gray-900">桶裝水訂單管理</h1>
      <p class="text-sm text-gray-500">管理桶裝水訂單與配送紀錄</p>
    </div>
    ...
    <div class="grid gap-4 md:grid-cols-4">
      <Card>
        ...
        <CardContent>
          <p class="text-3xl font-semibold text-gray-900">{{ totals.orders }}</p>
      ...
      <Card>
        ...
        <CardContent>
          <p class="text-3xl font-semibold text-amber-500">{{ totals.pending }}</p>
          <p class="text-xs text-gray-500">筆</p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <p class="text-3xl font-semibold text-green-600">{{ totals.completed }}</p>
          <p class="text-xs text-gray-500">筆</p>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent>
          <p class="text-3xl font-semibold text-gray-900">{{ formatCurrency(totals.revenue) }}</p>
    ...
    <TinyDialogBox v-model:visible="orderDialogVisible" :title="editOrderId ? '編輯桶裝水訂單' : '新增桶裝水訂單'" width="900px" class="max-h-[90vh]" :show-close="true">
      <template #footer>
        <div class="flex justify-end gap-2">
              ...
              <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div class="flex flex-1 flex-col gap-3 md:flex-row md:items-center md:gap-2">
                  <TinySelect
                    ...
                    :options="mockWaterProducts.map((item) => ({ label: `${item.name}（建議價 ${item.retailPrice}）`, value: item.id }))"
                  ...
                  <TinyInput
                    ...
                    @update:model-value="(val) => updateProductRow(product.uid, 'quantity', val)"
                  />
                  <TinyInput
                    class="md:w-28"
                    type="number"
                    placeholder="售價"
                    :model-value="product.actualPrice"
                    @update:model-value="(val) => updateProductRow(product.uid, 'actualPrice', val)"
                  />
                </div>
