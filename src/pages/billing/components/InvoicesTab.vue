<!-- src/pages/billing/components/InvoicesTab.vue 發票管理 -->
<template>
  <div class="flex flex-col gap-4">
    ...
    <div class="flex items-end gap-3 rounded-md bg-[#f5f7fb] p-4">
      <AForm layout="vertical">
        <div class="grid grid-cols-2 gap-x-2">
          <AFormItem :label="t('invoiceDate', '發票日期')">
          ...
          <AFormItem :label="t('search', '關鍵字')">
            <TinyInput v-model="filters.search" :placeholder="t('searchInvoice', '發票號碼/客戶')" @keyup.enter="handleFiltersChange" />
          </AFormItem>
        </div>
      </AForm>
    </div>
    ...
    <CustomTinyGrid :data="basicDataList" :height="systemStore.tableHeight" :border="true" row-key="id">
      ...
      <CustomTinyGridColumn field="remainingAmount" :title="t('remainingAmount', '未付金額')" width="130" align="right">
        <template #default="{ row }">
          <span :class="row.remainingAmount > 0 ? 'text-orange-600' : 'text-green-600'">NT$ {{ formatNumber(row.remainingAmount) }}</span>
        </template>
      </CustomTinyGridColumn>
  ...
  <!-- 新增/編輯發票彈窗 -->
  <a-modal v-model:visible="dialogVisible" :title="isEditing ? t('editInvoice', '編輯發票') : t('createInvoice', '新增發票')" width="800px" :mask-closable="false">
      ...
      <AForm ref="formRef" :model="basicForm" :rules="basicFormRules" layout="vertical" auto-label-width>
        ...
        <div class="mt-4">
          <div class="mb-2 flex items-center justify-between">
            <span class="font-medium">{{ t('invoiceItems', '發票項目') }}</span>
  ...
  <!-- 發票詳情 Drawer -->
  <a-drawer v-model:visible="detailDrawerVisible" :title="t('invoiceDetail', '發票詳情')" width="600px" :footer="false">
    <div v-if="selectedInvoice" class="space-y-4">
      ...
      <div class="rounded bg-gray-50 p-4">
        ...
        <div class="grid grid-cols-2 gap-3 text-sm">
          <div>
            <span class="text-gray-500">{{ t('invoiceNumber', '發票號碼') }}：</span>
      ...
      <!-- 發票項目 -->
      <div>
        <h3 class="mb-3 font-medium">{{ t('invoiceItems', '發票項目') }}</h3>
        <div class="space-y-2">
...
const _saveForm = async () => {
  ...
  if (basicForm.value.items.length === 0) {
    await mainStore.SWAL_Error(t('pleaseAddItems', '請新增至少一個發票項目'));
    return;
  }
...
const handleSendInvoice = async (row) => {
  const confirmed = await mainStore.SWAL_Confirm(t('confirmSendInvoice', '確定要發送此發票嗎？'));
  if (!confirmed) return;
...
const handleDeleteInvoice = async (row) => {
  const confirmed = await mainStore.SWAL_Confirm(t('confirmDeleteInvoice', '確定要刪除此發票嗎？'));
  if (!confirmed) return;
