...
<template>
  <div class="flex flex-col gap-4">
      ...
      <AForm layout="vertical">
        <div class="flex flex-wrap gap-3">
          ...
          <AFormItem :label="t('invoice', '發票')">
            <InfiniteSelect v-model="filters.invoiceId" dataSource="invoices" :placeholder="t('all', '全部')" allowClear class="w-48" @change="handleFiltersChange" />
          </AFormItem>
        </div>
      </AForm>
    </div>
    ...
    <CustomTinyGrid :data="basicDataList" :height="systemStore.tableHeight" :border="true" row-key="id">
      <CustomTinyGridColumn field="creditMemoNumber" :title="t('creditMemoNumber', '折讓單號')" width="160" fixed="left" />
      ...
      <CustomTinyGridColumn field="actions" :title="t('actions', '操作')" width="160" fixed="right">
        <template #default="{ row }">
          <div class="flex gap-2">
            <a-button v-if="row.status === 'DRAFT'" type="text" size="mini" status="success" @click="handleApplyCreditMemo(row)">{{ t("apply", "套用") }}</a-button>
            <a-button v-if="row.status === 'DRAFT'" type="text" size="mini" status="danger" @click="handleCancelCreditMemo(row)">{{ t("cancel", "取消") }}</a-button>
            <a-button v-if="row.status === 'APPLIED'" type="text" size="mini" status="danger" @click="handleCancelCreditMemo(row)">{{ t("void", "作廢") }}</a-button>
          </div>
        </template>
      </CustomTinyGridColumn>
    </CustomTinyGrid>
    ...
    <perfect-scrollbar class="h-[calc(100vh-400px)] pr-4">
      <AForm ref="formRef" :model="basicForm" :rules="basicFormRules" layout="vertical" auto-label-width>
        <AFormItem :label="t('invoice', '發票')" field="invoiceId">
          <InfiniteSelect v-model="basicForm.invoiceId" dataSource="invoices" :placeholder="t('pleaseSelect', '請選擇')" :filter-params="{ status: ['SENT', 'PARTIAL', 'PAID', 'OVERDUE'] }" @change="handleInvoiceChange" />
        </AFormItem>
...
import { CreditMemoListGet, CreditMemoCreatePost, CreditMemoGetById, CreditMemoApplyPost, CreditMemoCancelPost } from "@/assets/API/Billing";
...
const handleApplyCreditMemo = async (row) => {
  const confirmed = await mainStore.SWAL_Confirm(t("confirmApplyCreditMemo", "確定要套用此折讓單嗎？套用後將影響發票金額。"));
  if (!confirmed) return;
...
const handleCancelCreditMemo = async (row) => {
  ...
  const confirmed = await mainStore.SWAL_Confirm(message);
  if (!confirmed) return;
