    ...
    <CardContent class="flex flex-col gap-2 h-[calc(100vh-300px)]">
        ...
        <AForm layout="vertical">
          <div class="grid grid-cols-4 gap-4">
            ...
            <AFormItem :label="t('startDate', '起始日期')">
              <TinyDatePicker v-model="filters.reportDateFrom" :placeholder="t('selectDate', '請選擇日期')" value-format="yyyy-MM-dd" clearable @change="handleFiltersChange" />
            </AFormItem>
            <AFormItem :label="t('endDate', '結束日期')">
              <TinyDatePicker v-model="filters.reportDateTo" :placeholder="t('selectDate', '請選擇日期')" value-format="yyyy-MM-dd" clearable @change="handleFiltersChange" />
            </AFormItem>
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <a-button @click="openExportDialog">{{ t('匯出Excel', '匯出Excel') }}</a-button>
      ...
      <CustomTinyGrid :data="basicDataList" :height="systemStore.tableHeight" row-key="id" :expand-config="{ trigger: 'row', accordion: true }">
          ...
          <template #default="{ row }">
            <div class="bg-gray-50 p-4 flex flex-col gap-2">
              <div class="flex gap-2 items-center">
                ...
                <a-button status="success" @click="handleConvertToOrder(row)">
                  {{ t('轉入訂單', '轉入訂單') }}
                </a-button>
              </div>
        ...
        <CustomTinyGridColumn field="reportDate" :title="t('date', '日期')" width="120" sortable />
        <CustomTinyGridColumn field="weekDays" :title="t('deliveryDays', '出貨星期')" min-width="150" />
        <CustomTinyGridColumn field="totalAmount" :title="t('totalAmount', '總金額')" width="130" align="right" sortable>
        ...
        <CustomTinyGridColumn field="" :title="t('actions', '操作')" width="250" fixed="right" align="center">
          <template #default="{ row }">
            <div class="flex items-center justify-center gap-2">
              <a-button class="px-[2px]!" type="text" status="danger" @click="handleDelete(row)">{{ t('delete') }}</a-button>
              <a-button class="px-[2px]!" type="text" status="success" @click="handlePrint(row)">{{ t('print', '列印') }}</a-button>
              <a-button class="px-[2px]!" type="text" status="warning" @click="handleCopyLink(row)">{{ t('連結', '連結') }}</a-button>
              <a-button class="px-[2px]!" type="text" status="normal" @click="handleEdit(row)">{{ t('edit', '編輯') }}</a-button>
            </div>
          </template>
        </CustomTinyGridColumn>
      </CustomTinyGrid>
