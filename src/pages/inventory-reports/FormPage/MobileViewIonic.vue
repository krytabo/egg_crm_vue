  ...
  <ion-page>
    ...
    <ion-header>
      <ion-toolbar>
        ...
        <ion-title>{{ isCreateMode ? t('createReport', '新增報表') : t('editReport', '編輯報表') }}</ion-title>
      </ion-toolbar>
    </ion-header>
        ...
        <ion-card-content>
          <ion-list lines="none">
            ...
            <ion-item>
              <div class="weekday-chips">
                <ion-chip
                  ...
                  @click="toggleWeekDay(option.label)"
                  class="px-2"
                >
                  {{ option.text }}
                </ion-chip>
            ...
            <ion-item>
              ...
              <ion-textarea :placeholder="t('enterNote', '輸入備註...')" :value="basicForm.note" @ionInput="basicForm.note = $event.detail.value" auto-grow />
            </ion-item>
          </ion-list>
        </ion-card-content>
      </ion-card>
      ...
      <ion-card class="info-card">
        <ion-card-header class="p-3 gap-1">
          <ion-card-title class="flex items-center gap-2">
            ...
            <nut-button type="info" @click="openAddProductDialog"> <ion-icon slot="start" :icon="addOutline" />{{ t('new', '新增') }} </nut-button>
          </ion-card-title>
        </ion-card-header>
        <ion-card-content>
          <ion-list>
        ...
        <ion-card-content>
          <ion-list>
            ...
            <ion-item>
              ...
              <ion-input slot="end" placeholder="Enter text" :value="fuelExpense" @ionInput="fuelExpense = Number($event.detail.value) || 0" class="ion-text-right">
                <ion-note slot="end">元</ion-note>
              </ion-input>
            </ion-item>
            <ion-item>
              <ion-label>{{ t('otherExpense', '其他支出') }}</ion-label>
              <ion-input slot="end" placeholder="Enter text" :value="otherExpense" @ionInput="otherExpense = Number($event.detail.value) || 0" class="ion-text-right">
                <ion-note slot="end">元</ion-note>
              </ion-input>
            </ion-item>
          </ion-list>
    ...
    <ion-footer class="py-3 px-4 bg-white shadow-xl border-t-2">
      <ion-toolbar>
        ...
        <nut-button slot="end" type="info" :disabled="saving" @click="handleSave">
    ...
    <ion-modal :is-open="editPopupVisible" @didDismiss="closeEditPopup" :initial-breakpoint="0.75" :breakpoints="[0, 0.5, 0.75, 1]">
      ...
      <ion-content v-if="editingItem">
        <ion-list>
          ...
          <ion-item>
            <ion-label>{{ t('備註', '備註') }}</ion-label>
            <ion-input slot="end" :placeholder="t('請輸入備註', '請輸入備註')" :value="editingItem.note" @ionInput="editingItem.note = $event.detail.value" class="ion-text-right" />
          </ion-item>
        </ion-list>
    ...
    <ion-modal :is-open="addProductDialogVisible" @didDismiss="closeAddProductDialog" :initial-breakpoint="0.75" :breakpoints="[0, 0.5, 0.75, 1]">
      ...
      <ion-content>
        <ion-list>
          ...
          <ion-item>
            <ion-label>{{ t('備註', '備註') }}</ion-label>
            <ion-input slot="end" :placeholder="t('請輸入備註', '請輸入備註')" :value="newProductNote" @ionInput="newProductNote = $event.detail.value" class="ion-text-right" />
          </ion-item>
        </ion-list>
