<!-- src/pages/inventory-reports/InventoryManagementPage.vue 庫存管理 -->
<template>
  <Card>
    <!--＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝-->
    <!--         Tab 頁籤          -->
      ...
      <template #extra>
        <div class="flex items-center gap-3">
          ...
          <template v-if="isOverview">
            ...
            <a-button type="primary" @click="openCycleCountDialog">{{ t("cycleCount", "盤點作業") }}</a-button>
          </template>

          <!-- 異動記錄 Tab 操作按鈕 -->
          <template v-if="isMovements">
            <a-button status="danger" plain @click="movementsTabRef?.clearFilter()">{{ t("clearAllSearch", "清除篩選") }}</a-button>
            <a-button type="primary" @click="movementsTabRef?.openCreateDialog()">{{ t("addTransaction", "新增異動") }}</a-button>
          </template>

          <!-- 庫存警示 Tab 操作按鈕 -->
          <template v-if="isAlerts">
            <a-button type="primary" @click="alertsTabRef?.openQuickStockIn()">{{ t("add", "新增") }}</a-button>
          </template>

          <!-- 庫存調撥 Tab 操作按鈕 -->
          <template v-if="isTransfer">
            <a-button status="danger" plain @click="transferTabRef?.clearFilter()">{{ t("clearAllSearch", "清除篩選") }}</a-button>
            <a-button type="primary" @click="transferTabRef?.openTransferDialog()">{{ t("newTransfer", "新增調撥") }}</a-button>
          </template>
